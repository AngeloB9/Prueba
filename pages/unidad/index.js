...
import CreateUnidad from '@/components/forms/createUnidad';
...
const index = ({ user }) => {
  ...
  const handleSubmit = async (values) => {
    setloading(true);
    try {
      const response = await axios(user.token).post('/unidades', values);
  ...
  return (
    <AdminLayout user={user}>
      ...
      <CreateUnidad handleSubmit={handleSubmit} unidad={unidad} />
      {/*----------Modal de petición exitosa------- */}
      <ModalSuccess
        show={modalSuccess}
        handleClose={() => setmodalSuccess(false)}
        tituloMensaje='Creación Exitosa'
        mensaje='La unidad se ha ingresado satisfactoriamente!'
