...
import EditProveedor from '@/components/forms/editProveedor';
...
export const getServerSideProps = withSession(
  async ({ query: { id }, req, res }) => {
    ...
    const { data: proveedorEdit } = await axios(user.token).get(
      `/proveedores/${id}`
    );
...
const index = ({ user, proveedorEdit, id }) => {
  ...
  const handleSubmit = async (values) => {
    setloading(true);
    try {
      const response = await axios(user.token).put(`/proveedores/${id}`, values);
  ...
  return (
    <AdminLayout user={user}>
      ...
      <EditProveedor handleSubmit={handleSubmit} proveedor={proveedorEdit} />
      {/*----------Modal de petición exitosa------- */}
      <ModalSuccess
        show={modalSuccess}
        handleClose={() => setmodalSuccess(false)}
        tituloMensaje='Edición Exitosa'
        mensaje='El proveedor se ha editado satisfactoriamente!'
