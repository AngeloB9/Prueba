...
const index = ({ user }) => {
  ...
  const handleSubmit = async (values) => {
    setloading(true);
    try {
      const response = await axios(user.token).post('/categorias', values);
  ...
  return (
    <AdminLayout user={user}>
      ...
      <CreateCategory handleSubmit={handleSubmit} category={category} />
      {/*----------Modal de petición exitosa------- */}
      <ModalSuccess
        show={modalSuccess}
        handleClose={() => setmodalSuccess(false)}
        tituloMensaje='Creación Exitosa'
        mensaje='La categoria se ha ingresado satisfactoriamente!'
