...
const schema = yup.object().shape({
  ...
  correo_personal: yup.string().email('Correo Inválido').required('Campo requerido'),
  informacion_adicional: yup.string().required('Campo requerido'),
});

const CreateOperador = ({ handleSubmit, operador }) => {
  return (
    <div className='p-4'>
      <h4 className='mb-3'>Nuevo Operador</h4>
      <Formik
        validationSchema={schema}
        onSubmit={(values, actions) => {
          handleSubmit(values);
        }}
        initialValues={operador}>
